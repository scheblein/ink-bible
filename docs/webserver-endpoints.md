...
available while Ink Bible is in File Transfer or Calibre Wireless mode.
...
Examples use `inkbible.local`. If mDNS does not resolve on your network, use
...
```bash
curl http://inkbible.local/api/status
```
...
```bash
curl "http://inkbible.local/api/files?path=/Books"
```
...
```bash
curl -OJ "http://inkbible.local/download?path=/Books/MyBook.epub"
```
...
```bash
curl -X POST -F "file=@mybook.epub" "http://inkbible.local/upload?path=/Books"
```
...
```bash
curl -X POST -d "name=NewFolder&path=/" http://inkbible.local/mkdir
```
...
```bash
curl -X POST -d "path=/Books/old.epub&name=new.epub" http://inkbible.local/rename
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub&dest=/Read" http://inkbible.local/move
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub" http://inkbible.local/delete
curl -X POST -d 'paths=["/Books/old.epub","/OldFolder"]' http://inkbible.local/delete
```
...
```bash
curl http://inkbible.local/api/settings
```
...
curl -X POST \
  ...
  http://inkbible.local/api/settings
```
...
```bash
curl http://inkbible.local/api/fonts
```
...
curl -X POST \
  ...
  http://inkbible.local/api/fonts/upload
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"family":"Literata"}' \
  http://inkbible.local/api/fonts/delete
```
...
```bash
curl http://inkbible.local/api/opds
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://inkbible.local/api/opds/delete
```
...
```bash
curl http://inkbible.local/api/wifi
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://inkbible.local/api/wifi/delete
```
...
- `inkbible.local` is advertised with mDNS when available.
- `/api/status` returns `"mode": "STA"` and RSSI in dBm.

### Access Point Mode (AP)

- Device creates an open hotspot named `Ink-Bible`.

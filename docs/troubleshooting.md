...
1. Verify both devices are on the correct network
   ...
   - In **Join Network** mode, your computer/phone and Ink Bible must be on the same Wi-Fi network
   - In **Create Hotspot** mode, your computer/phone must be connected to the `Ink-Bible` hotspot
2. Double-check the IP address
   - Make sure you typed it correctly
   - Include `http://` at the beginning
   - Try the displayed IP address if `http://inkbible.local/` does not resolve
